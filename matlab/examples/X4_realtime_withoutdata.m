...
while ishandle(fh)
    ...
    drawnow;
    threshold = 10; % setting recognition gap
    [Infocache,output] = Signalupdate(iq_vec, Infocache, 90, threshold, FPS);
    
    if size(output,1) >= 1
        [resp_rate, heart_rate] = resp_detect(output,FPS);
...
if size(storage,2) == threshold_bin
    ...
    cache = analysis_signal(:,1:(threshold_bin-2));
    
    pca_coef = pca(analysis_signal(10:channel,:)');
    output = (analysis_signal(10:channel,:)') * pca_coef(:,1);
else 
    cache = [Input_signal,storage];
    output = [];
end
end
function [resp_rate, heart_rate] = resp_detect(signal,fs_sample)
%% PCA data plot
...
resp_sig = IIR_BP(angle(signal(:,:)),fs_sample,[0.1 0.5]);% 
plot(t,resp_sig(:,:),'LineWidth',3);
...
heart_sig = IIR_BP(angle(signal(:,:)),fs_sample,[1 2]);% 

plot(t,heart_sig(:,:),'r','LineWidth',3);
ylabel('Magnitude');  xlabel('Time (s)');
xlim([0,max(t)]);
grid on;
...
[resp_rate,~] = Rate_estimation(resp_sig,fs_sample,'r',blechar);
figure(5);
[heart_rate,~] = Rate_estimation(heart_sig,fs_sample,'h',blechar);
%subplot(211)
