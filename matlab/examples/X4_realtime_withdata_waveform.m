...
function [resp_list,heart_list,Timelist] = X4_realtime_withdata(record_duration,record_person,record_activity,COMPORT)
...
output_dir = 'C:\Users\2288980g\Desktop\XeThru Recordings\'
...
%     
%     drawnow;
    threshold = 20; % setting recognition gap
    ...
    if size(output,1) >= 1
        ...
        [resp_rate, heart_rate, a1, a2 ,a3, a4] = resp_detect(output,FPS,hr,a3,a4);
        %resp_list = [resp_list, resp_rate]; %Store the vitals detection result
        %heart_list = [heart_list, heart_rate];
        %Timelist = [Timelist, toc(tStart)];
...
if size(storage,2) == threshold_bin
    ...
    cache = analysis_signal(:,1:(threshold_bin-10));
    
    pca_coef = pca(analysis_signal(1:channel,:)');
    output = (analysis_signal(1:channel,:)') * pca_coef(:,1);
else 
    cache = [Input_signal,storage];
    output = [];
end
end
function [resp_rate, heart_rate ,a1,a2,a3,a4] = resp_detect(signal,fs_sample,blechar,a3,a4)
%% PCA data plot
...
[resp_rate,~,a1,a2] = Rate_estimation_pk(resp_sig,fs_sample,'r',blechar);
subplot(212)
delete(a3);
delete(a4);
[~,heart_rate,a3,a4] = Rate_estimation(heart_sig,fs_sample,'h',blechar);
%subplot(211)
%[resp_rate, ref_rr] = Rate_estimation_fft(resp_sig,fs_sample,'r',blechar);
%subplot(212)
%[heart_rate, ref_hr] = Rate_estimation_fft(heart_sig,fs_sample,'h',blechar);
