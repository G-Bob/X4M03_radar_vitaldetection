...
while ishandle(fh)
    ...
    if size(output,1) >= 1
        [resp_rate, heart_rate, ref_rr, ref_hr] = resp_detect(output,FPS,hr);
...
if size(storage,2) == threshold_bin
    ...
    cache = analysis_signal(:,1:(threshold_bin-50));
...
function [resp_rate, heart_rate, ref_rr, ref_hr] = resp_detect(signal,fs_sample,blechar)
%% PCA data plot
